...
public class CategoryRepositoryTests
{
    ...
    public async Task GetAllByRestaurantAsync_ReturnsAllIncludingHidden()
    {
        var restaurantId = Guid.NewGuid();
        _db.Categories.AddRange(
            MakeCategory("Visible", restaurantId),
    ...
    public async Task GetVisibleByRestaurantAsync_DoesNotReturnOtherRestaurantCategories()
    {
        ...
        var result = await _repo.GetVisibleByRestaurantAsync(Guid.NewGuid());
    ...
    {
        await _repo.AddAsync(MakeCategory("New"));
    ...
    public async Task DeleteAsync_RemovesCategory()
    {
        ...
        Assert.That(await _repo.DeleteAsync(category.Id), Is.True);
        Assert.That(await _db.Categories.AnyAsync(c => c.Id == category.Id), Is.False);
    }

    [Test]
    public async Task HasActiveItemsAsync_WhenActiveItemsExist_ReturnsTrue()
    {
        var restaurantId = Guid.NewGuid();
        ...
        Assert.That(await _repo.HasActiveItemsAsync(category.Id), Is.True);
    }
    ...
    public async Task HasActiveItemsAsync_WhenOnlySoftDeletedItems_ReturnsFalse()
    {
        ...
        Assert.That(await _repo.HasActiveItemsAsync(category.Id), Is.False);
    }
